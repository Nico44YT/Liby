    ...
    public static void drawShape(LibyParticleData data, int maxPoints, Vec3d ankerPoint, Vec3d... points) {
        for(int i = 0;i<points.length;i++) {
            ...
            for(int j = 0;j<maxPoints;j++) {
                ...
                if(data.important) data.world.addImportantParticle(data.particleEffect, data.alwaysSpawns, point.x, point.y, point.z, data.velocity.x, data.velocity.y, data.velocity.z);
                else data.world.addParticle(data.particleEffect, data.alwaysSpawns, point.x, point.y, point.z, data.velocity.x, data.velocity.y, data.velocity.z);
            }
        }
    }
